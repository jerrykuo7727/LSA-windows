...
print 'Size of QUS = {0}'.format(qt_mult_lsaU.shape)
print 'Size of lsa_Vt = {0}'.format(lsa_Vt.shape)


for q in range(0, len(qt_mult_lsaU)):

    temp_QUS_cos_every_row_ofVt = [0]*2266
    for v in range(0, len(lsa_Vt)):
        molecule = np.dot(lsa_Vt[v], qt_mult_lsaU[q]) #分子 Molecule
        denominator = la.norm(lsa_Vt[v])*la.norm(qt_mult_lsaU[q]) #分母 Denominator
